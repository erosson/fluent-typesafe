...
async function main(resources: Array<[string, Promise<Resource>]>, write: (path: string, data: string) => Promise<void>): Promise<void> {
    const modules: ModuleOutput[] = await Promise.all(resources.map(async ([shortPath, pending]) => {
        ...
        const output = await elmFormat(genResource(resource.messages, moduleName, isNamespaced ? namespace : null))
...
function genResource(messages: Message[], moduleName: string, namespace: string | null) {
...
${messages.map(m => genMessage(m, namespace)).join("\n\n")}
`
    )
}
function genMessage(message: Message, namespace: string | null) {
