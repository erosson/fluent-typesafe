...
function genMessage(message: Message) {
    if (message.placeholders.length) {
        return (`\
export function ${upperFirst(camelCase(message.id))}(props: {children?: React.ReactNode, elems?: {[name: string]: JSX.Element}, vars: {${message.placeholders.map(genArgType).join(', ')}}}): JSX.Element {
...
`)
        ...
        return (`\
export function ${upperFirst(camelCase(message.id))}(props: {children?: React.ReactNode, elems?: {[name: string]: JSX.Element}}): JSX.Element {
