                            ...
                            return __generator(this, function (_c) {
                                switch (_c.label) {
                                    ...
                                    case 1:
                                        ...
                                        return [4 /*yield*/, prettier(genResource(resource.messages, isNamespaced ? namespace : null))];
                                    case 2:
                                        output = _c.sent();
                                        return [4 /*yield*/, write(outputPath + ".tsx", output)];
                                    case 3:
                                        _c.sent();
...
function genResource(messages, namespace) {
...
function genMessage(message, namespace) {
    var name = (0, lodash_1.upperFirst)((0, lodash_1.camelCase)(message.id.slice((namespace || '').length)));
    return ("export function " + name + "(props: __props" + genVars(message.placeholders) + genAttrs(message.attributes) + "): JSX.Element {\n    return <__L.Localized id=\"" + message.id + "\" {...props} />\n}\n");
}
...
function genAttrs(attributes) {
    return attributes.size
        ? " & {attrs: {" + Array.from(attributes.values()).map(function (a) { return JSON.stringify(a) + ": boolean"; }).join(', ') + "}}"
        : '';
}
