...
function genMessage(message, namespace) {
    ...
    if (message.placeholders.length) {
        return ("export function " + name + "(args: {" + message.placeholders.map(genArgType).join(', ') + "}): {'data-l10n-id': string, 'data-l10n-args': string } {\n    return {\n        \"data-l10n-id\": " + JSON.stringify(message.id) + ",\n        \"data-l10n-args\": JSON.stringify({" + message.placeholders.map(genArgEncoder).map(function (arg) { return "\n            " + arg + ","; }).join('') + "\n        })\n    }\n}\n");
    }
    else {
        return ("export const " + name + ": {'data-l10n-id': string} =\n    {\"data-l10n-id\": " + JSON.stringify(message.id) + "}\n");
    }
}
